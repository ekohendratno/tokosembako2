						...
						$.ajax({
							...
							success: function(data) {
								...
								switch (data.status) {
									case 1:
										gbox.hide();
										$('#menu-group').append('<li><a href="' + site_url('?opfo&sys=appearance&go=menus&group_id=' + data.id) + '">' + group_title + '</a></li>');
										break;
									case 2:
			...
			buttons: {
				'Yes': function() {
					$.post(site_url('?request&load=libs/ajax/menu-group.php&aksi=delete'), param, function(data) {
						if (data.success) {
							window.location = site_url('?opfo&sys=appearance&go=menus');
						} else {
							gbox.show({
								content: '<div class="padding">Failed to delete this menu.</div>'
							});
